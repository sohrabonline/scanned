...
public class ScanView: UIView,AVCaptureMetadataOutputObjectsDelegate,FlutterPlugin {
  ...
  init(_ frame:CGRect, viewId:Int64, args: Any?,registrar: FlutterPluginRegistrar) {
    ...
    self.queue = DispatchQueue.init(label: "com.shrapp.scanned", attributes: .concurrent);
    self.session = AVCaptureSession();
    self.channel = FlutterMethodChannel(name: "scanned/method_\(viewId)", binaryMessenger: registrar.messenger());
...
//      }
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
      ...
      case "pause":
          ...
          result(false)
      case "turnOn":
          let success = self.turnOn()
          result(success)
      case "turnOff":
          let success = self.turnOff()
          result(success)
      case "toggleTorchMode":
          let success = self.toggleTorchMode() // Corrected method name
          result(success)
      default:
    ...
    private func turnOn() -> Bool {
        if let device = AVCaptureDevice.default(for: AVMediaType.video) {
            do {
                ...
                print("Failed to turn flash on: \(error.localizedDescription)")
                return false
            }
        }
        return false
    ...
    private func turnOff() -> Bool {
        if let device = AVCaptureDevice.default(for: AVMediaType.video) {
            do {
                ...
                print("Failed to turn flash off: \(error.localizedDescription)")
                return false
            }
        }
        return false
            ...
            private func toggleTorchMode() -> Bool {
                if torchActive {
                    if turnOff() {
                        torchActive = false
                        return false // Flash is off
                    }
                } else {
                    if turnOn() {
                        torchActive = true
                        return true // Flash is on
                    }
                }
                return torchActive // Unchanged
            }
}
